...
function fieldStyle(focused: boolean, hasError: boolean) {
  return {
...
export function RegisterForm() {
  const [state, action, pending] = useActionState(registerCompany, undefined);
    ...
    <form action={action} className="flex flex-col gap-5">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div
          ...
          style={fieldStyle(focused === "company", !!state?.errors?.companyName)}
        >
        ...
        {state?.errors?.companyName?.[0] && (
          <p className="text-xs text-red-500">{state.errors.companyName[0]}</p>
        )}
      </div>
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div
          className="flex items-center h-11 rounded-xl bg-white overflow-hidden"
          style={fieldStyle(focused === "email", !!state?.errors?.email)}
        >
          <span className="pl-3.5 text-slate-400 shrink-0">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div
          className="flex items-center h-11 rounded-xl bg-white overflow-hidden"
          style={fieldStyle(focused === "password", !!state?.errors?.password)}
        >
          <span className="pl-3.5 text-slate-400 shrink-0">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        {state?.errors?.password?.filter((e) =>
          !(e.includes("minimal 8") && password.length >= 8) &&
          !(e.includes("huruf kapital") && /[A-Z]/.test(password)) &&
          !(e.includes("angka") && /[0-9]/.test(password))
        ).map((e, i) => (
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div
          className="flex items-center h-11 rounded-xl bg-white overflow-hidden"
          style={fieldStyle(focused === "confirm", passwordMismatch || !!state?.errors?.confirmPassword)}
        >
          <span className="pl-3.5 text-slate-400 shrink-0">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <rect width="18" height="11" x="3" y="11" rx="2" ry="2" />
        ...
        {(passwordMismatch || state?.errors?.confirmPassword?.[0]) && (
          <p className="text-xs text-red-500">
            {passwordMismatch ? "Password tidak cocok" : state!.errors!.confirmPassword![0]}
          </p>
        )}
      </div>
      ...
      >
        {pending ? <><Spinner /><span>Mendaftar...</span></> : "Daftar Sekarang"}
      </button>
